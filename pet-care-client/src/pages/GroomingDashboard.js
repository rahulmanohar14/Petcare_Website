...
function GroomingDashboard() {
  ...
  const fetchAppointments = async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/grooming?email=${user.email}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${API_BASE}/api/grooming`, {
  ...
  const handleCancel = async (id) => {
    if (window.confirm('Cancel this appointment?')) {
      await axios.delete(`${API_BASE}/api/grooming/${id}`);
  ...
  const handleUpdate = async () => {
    await axios.put(`${API_BASE}/api/grooming/${editId}`, editData);
