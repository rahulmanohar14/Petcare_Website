...
function AdoptionManagerDashboard() {
  ...
  const fetchRequests = async () => {
    const res = await axios.get(`${API_BASE}/api/adoptions`);
  ...
  const handleAddPet = async () => {
    ...
    try {
      await axios.post(`${API_BASE}/api/pets`, newPet);
  ...
  const handleApprove = async (id) => {
    await axios.put(`${API_BASE}/api/adoptions/${id}`, { status: 'Approved' });
    setSnackbar({ open: true, message: 'Request approved', severity: 'success' });
    fetchRequests();
  };

  const handleReject = async (id) => {
    await axios.put(`${API_BASE}/api/adoptions/${id}`, { status: 'Rejected' });
